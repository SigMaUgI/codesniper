...
local WHITELIST_URL = "https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/refs/heads/main/whitelist.json"
...
local function isWhitelisted()
    ...
    if type(data.allowed_user_ids) == "table" then
        for _, id in ipairs(data.allowed_user_ids) do
    ...
    if type(data.allowed_usernames) == "table" then
        for _, name in ipairs(data.allowed_usernames) do
            if string.lower(tostring(name)) == username then
                return true
            end
        end
    end
...
-- NOTHING from CodeSniper runs unless access is present.
