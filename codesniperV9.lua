...
local function CheckWhitelist(noCache)
    ...
    -- Match UserId first. JSON numbers and JSON strings both work.
    if type(data.allowed_user_ids) == "table" then
        for _, id in pairs(data.allowed_user_ids) do
            if tostring(id):gsub("%s+", "") == myId then
                return true, "USER_ID"
            end
        end
    end

    -- Username is a fallback. Case and accidental spaces do not matter.
    if type(data.allowed_usernames) == "table" then
        for _, name in pairs(data.allowed_usernames) do
            if Normalize(name) == myName then
